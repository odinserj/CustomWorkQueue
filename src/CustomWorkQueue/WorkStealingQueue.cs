...
[assembly: InternalsVisibleTo("CustomWorkQueue.Benchmarks")]

namespace CustomWorkQueue
{
    internal sealed class WorkStealingQueue<T> where T : class
    {
        // Small optimization to avoid calls to JIT_Ldelema_Ref
        ...
        internal volatile Element[] m_array = new Element[INITIAL_SIZE];
        ...
        public void LocalPush(T obj)
        {
            ...
            {
                Volatile.Write(ref m_array[tail & m_mask].Value, obj);
                m_tailIndex = tail + 1;
            }
            else
            {
                ...
                try
                {
                    ...
                    if (count >= m_mask)
                    {
                        ...
                        var newArray = new Element[m_array.Length << 1];
        ...
        public bool LocalFindAndPop(T obj)
        {
            // Fast path: check the tail. If equal, we can skip the lock.
            if (m_array[(m_tailIndex - 1) & m_mask].Value == obj)
            {
                T unused = LocalPop();
            ...
            for (int i = m_tailIndex - 2; i >= m_headIndex; i--)
            {
                if (m_array[i & m_mask].Value == obj)
                {
                    ...
                    try
                    {
                        ...
                        if (m_array[i & m_mask].Value == null)
                            return false;

                        // Otherwise, null out the element.
                        Volatile.Write(ref m_array[i & m_mask].Value, null);
        ...
        public T LocalPop() => m_headIndex < m_tailIndex ? LocalPopCore() : null;

        private T LocalPopCore()
        {
            while (true)
            {
                int tail = m_tailIndex;
